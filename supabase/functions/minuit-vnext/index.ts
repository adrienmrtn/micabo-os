 ...
 *
 *   {}  → kick rattrapage-elo (async) + assignation + upscale + burn
 *   { etapes?: ['stats'|'scores'|'assignation'|'upscale'|'burn'|'variations'|'rattrapage'], compteId?, date?, forcer? }
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const etapes: string[] = Array.isArray(body?.etapes)
      ? body.etapes
      : ["rattrapage", "assignation", "upscale", "burn"];
