-- On repromeut « micabo », le nom seul : ni « .app », ni mot de catégorie.
--
-- Rejoue 0260, que 0261 avait annulé. Troisième passage sur ce texte en deux
-- jours — d'où 0262, qui garde l'avant sous l'étiquette
-- `avant_micabo_nu_2026_09_15` pour que le prochain retour soit une
-- restitution exacte et non une reconstruction.
--
-- `micabo.app` reste le domaine de messagerie interne et l'URL de l'OS : la
-- règle les met de côté avant de toucher au texte.
...
-- 2b. Pertinence. 0261 l'avait passé à « micabo.app » partout, donc la chaîne
--     de 0260 n'y matcherait plus : passe générique, puis la description.
update public.prompts
set contenu = replace(public.micabo_reecrire_cta(contenu),
  $a$micabo est un site d'éducation IA pour les étudiants$a$,
  $b$micabo est une application mobile d'éducation IA pour les étudiants$b$)
where cle = 'pertinence_micabo';

-- 0261 avait aussi déplacé cette ligne de traduction_tr.
update public.prompts set contenu = replace(contenu,
  $a$Le remplacement par micabo.app est geré par un autre prompt$a$,
  $b$Le remplacement par micabo est geré par un autre prompt$b$)
