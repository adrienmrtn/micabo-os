/**
 * « Oublier » un compte source : règles pures de ciblage (miroir du backend).
 *
 * Un compte peut avoir laissé des traces même après une suppression classique
 * (les FK sont en ON DELETE SET NULL) : des slideshows dont `compte_reference_id`
 * est déjà nul, mais dont l'URL trahit encore le compte. On les retrouve par le
 * handle contenu dans `source_url`, et c'est ce qui débloque un ré-import propre
 * (`contenus.source_url` est unique).
 */
